...
Working draft for the ACCESS MAXIMIZE proposal, section 2.10 (Grady). Citations use section-local IEEE bracket numbers keyed to the reference list below; renumber into the main document's global sequence at merge (rendered there as superscripts via the document's cite style). Structure follows OUTLINE.md: Introduction → obstacle and prior work → proposed work → resource request.
...
| AIMD trajectories (8 compositions × 5 temperatures × 5 seeds, 10 ps each) | 200 | 6,144 | 1,228,800 |
| Active-learning relabeling (single points) | 2,000 | 32 | 64,000 |
| **Total, Bridges-2 RM** | | | **1,456,640** |

**NCSA Delta — GPU**

| Item | Count | GPU hours each | Sub-total (GPU h) |
|---|---:|---:|---:|
| MLIP training (active-learning generations) | 3 | 1,000 | 3,000 |
| Track simulations (stoichiometry/pre-damage series × stopping powers) | 30 | 100 | 3,000 |
| Annealing simulations | 10 | 100 | 1,000 |
| **Total, NCSA Delta GPU** | | | **7,000** |

Screening costs scale from our measured Bridges-2 timings for the reference calculations: 64 cores × 4 hours per 96-atom $\Gamma$-point relaxation gives 256 SUs, and production re-relaxations at the 2×2×2 k-mesh require 64 cores × 16 hours = 1,024 SUs. The AIMD estimate adopts the group's benchmark of 6,144 SUs (64 cores × 96 hours) per 10 ps trajectory for comparable oxide supercells; 200 trajectories give 200 × 6,144 = 1,228,800 SUs. MLIP training follows the group's Allegro benchmark of 10 GPU hours per 5 epochs, with ~500 epochs per active-learning generation. Track simulations at the $10^6$-atom, ~200 ps scale are estimated at 100 GPU hours each from Allegro throughput scaling.

<!-- TODO before submission: (1) confirm Delta GPU-hour → ACCESS credit conversion and check the grand total against the 1.5–2M credit target; (2) decide whether to replace the two scaling estimates (96-atom screening SU cost; fluorite AIMD SU cost) with measured benchmarks; (3) Figure N. -->
